...
function ___dom(){
    ...
    function _$(k, s, a) {
        ...
        if (typeof k === 'string')
            e = k.indexOf("#") != -1
                    ? document.getElementById(k.split('#')[1])
     ...
     */
    __prototype__.reset = {
        prototypeOf : [HTMLTextAreaElement, HTMLInputElement,
                HTMLSelectElement],
        handle : function() {
    ...
    __prototype__.resetDirty = {
        prototypeOf : [HTMLFormElement, HTMLTextAreaElement, HTMLInputElement,
                HTMLSelectElement],
        handle : function() {
            if (this instanceof HTMLFormElement) {
        ...
        handles : {
            confirmDisposedClose : function(cb) {
                if (this.confirmed === true)
                    ...
                    ca.ui
                            .confirm(
                                    'Devam ederseniz formdaki değişiklikleri kaybedeceksiniz.<br>Devam edilsin mi ?',
                                    function(ok) {
                                        this.confirmed = ok;
                                        cb(ok);
                                    }.bind(this), {
            ...
            confirmClose : function(cb, terminate) {
                ...
                ca.ui
                        .confirm(
                                'Devam ederseniz formdaki değişiklikleri<br>kaybedeceksiniz. Devam edilsin mi ?',
                                function(cb2, terminate2, ok) {
            ...
            addObject : function(o, n, g, fill) {
                ...
                if (o instanceof Function
                        || (o && o.__proto__ && o.__proto__ == Function.prototype)) {
                } else if (o && o.__proto__ && o.__proto__ == Object.prototype) {
                    for ( var k in o) {
                        var e = o[k];
                        this.addObject(e,
                                n ? n + '[' + k + ']' : '[' + k + ']', g, fill);
            ...
            getParams : function() {
                ...
                for (var i = 0, aLen = items.length; i < aLen; ++i) {
                    ...
                    if (item.type == "button" || item.excluded)
                        continue;
                    var name = item.name, value = item.getValue();
            ...
            getObject : function(n, a, prop) {
                ...
                // console.warn('GetObject items length', items.length);
                for (var i = 0; i < items.length; i++) {
                    var item = items[i];
                    if (item.type == "button" || item.excluded)
                        continue;
                    ...
                    if (!nms)
                        continue;
                    var rt = a || nms[1];
                    // console.warn('GetObject item index', i);
                    if (typeof nms[2] == 'undefined') {
                        ...
                        // console.log('getobject literal set value',rt, v,o
                        // );
                    } else {
                        // not scalar
                        var cs = rt ? [rt] : [];
                        var ms, sg;
                        // ms = rgx.exec(item.name);
                        while ((ms = rgx.exec(item.name)) !== null)
                            cs.push(ms[1]);
                        // o.backSet=undefined;
                        // delete o.backSet;
                        var p = o;
                        // console.info('getobject', cs, p);
                        for (var k = 0; k < cs.length; k++) {
                            ...
                            if (isA(p)) {
                                ...
                                // console.log('has', sg, p);
                                if (p.backSet && pp && pp.__proto__
                                        && pp.__proto__ !== Object.prototype) {
                                    p = p.backSet(sg, pp);
                                }
                                var bf = backSet.bind(p, sg, pp);
                                pp = p[sg];
    ...
    __prototype__.validate = {
        ...
        /**
         * looks up divs with validate attribute and match it with input with
         * same name sets their class as validator
         * 
         * @deprecated
         */
        handle : function() {
            // console.info('validating ', this);
            var p = this.parentNode, form = this.form, eName = this
                    .getAttribute('name');
            this.isValid = true;
                ...
                if (validatorHandle instanceof Function) {
                    ...
                    if (compare || pat) {
                        ...
                        if (valid == false) {
                            ...
                            window.scrollTo(0,
                                    parseInt((this.offsetPos.top - 60) / 2));
                        }
                        this.isValid = valid;
                    }/*
                         * set a function for manual pop
                         */
                    else {
                    }
                }
    ...
    __prototype__.maskDigit = {
        prototypeOf : HTMLInputElement,
        /**
         * Rules digit input and use setError for fault entry Use onkeypress
         * 
    ...
    __prototype__.spin = {
        prototypeOf : HTMLInputElement,
        handle : /**
                     * Set className 'spin' and disables form
                     * 
                     * @function
    ...
    __prototype__.setKeyPress = {
         ...
         * @param {Object}
         *            assgn Element that click event will be used
         * @param {Function}
         *            hnd
         * 
         */
    ...
    toolTip = function(opts) {
        ...
        var _a = arguments;
        var opts = _a[0], side = _a[1], pos = _a[02], tmo = _a[3], cls = _a[4], hideEvent = _a[5];
        var msg = opts;
        var style = {};
        ...
        if (this.parentElement.firstElementChild
                && this.parentElement.firstElementChild.hasClass(cls))
            return;
        ...
        function position(box, side, pos) {
            ...
            if (pos && (side == 0 || side == 2)) {
                // left += parseInt(((rc.width -
                // parseInt(getComputedStyle(box).width)) * pos) / 100);
            }
            ...
            // (bs.paddingTop.length?parseInt(bs.paddingTop):0) + 8);
    ...
    hideTip = function(t, tmo) {
        ...
        return setTimeout(
                (function() {
                    this
                            .css('transition: opacity 0.7s ease-in-out; opacity: 0;transition: opacity 0.7s ease-in-out; opacity: 0;');
    ...
    __prototype__.setToolTip = function(opts) {
        var msg = arguments[0], pos = arguments[1], hh = arguments[2], cls = arguments[3];
    ...
    __prototype__.setError = function(opts) {
        var msg = arguments[0], pos = arguments[1], tmo = arguments[2];
        var args;
        if (opts instanceof Object) {
    ...
    if(args.scroll!==false) this.scrollIntoView();
        ca.event.raise('error-set', {
            message : msg
        }, this, true);
    };
    /**
     * <div class="hyper-link upload-file" data-control-on-ready="function(){
     ...
     * 
     * @param {object}
     *            args { debug : false, filter : 'jpg,jpeg,gif,png', overwrite :
     *            false, maxLength : -1, temporary : true, }
     * @memberOf __prototype__
     */
    __prototype__.initializeUpload = {
        ...
        handle : function(args) {
            ...
            if (typeof args !== 'object') {
                console
                        .warn(
                                'you can use parameters for initializeUpload default options will be used',
                                options);
            } else {
                ...
                for ( var property in options) {
                    if (!args.hasOwnProperty(property)) {
                        console.info(property
                                + ' not defined in args, default value "'
                                + options[property] + '" will be used');
                    }
                }
                console.groupEnd();
                options.extend(args);
            ...
            function ih(args, e) {
                var a = {
                    ...
                    callBack : function(options, target) {
                        // cb.call(this.uploadResponse, args.params,
                        // args.owner);
    ...
    /*
     * Theme Rule name: storage.type; scope: storage.type; style: Foreground:
     * {0, 0, 192, 255}, Style: 1
     */
    /**
     * @memberOf __prototype__
    ...
    __prototype__.disable = function(on, progress) {
        ...
        if (isInputElement(this)) {
            ...
            if ('object' === typeof progress && progress.cursor) {
                if (on)
                    this.defaultCursor = progress.cursor[1]
                            || this.computedStyle.cursor;
                this.style.cursor = on
                        ? progress.cursor[0] || 'wait'
                        : (this.defaultCursor && this.defaultCursor.length)
                                || 'default';
            }
            return;
        }
        ...
        if (on == true) {
            ...
            if (progress === false)
                ...
                if (on && 'undefined' === typeof n.defaultCursor)
                    n.defaultCursor = progress.cursor[1]
                            || n.computedStyle.cursor;
                n.style.cursor = on
                        ? progress.cursor[0] || 'wait'
                        : (n.defaultCursor && n.defaultCursor.length)
                                || 'default';
            } else {
            ...
            function rePos(n) {
                ...
                if (rc) {
                    n.css({
                        ...
                        backgroundPosition : '50% '
                                + (n.scrollHeight
                                        ? parseInt(n.scrollHeight / 2) + 'px'
                                        : '50%')
                    });
                }
                // n.css('top:' + parseInt(p.offsetTop) + 'px !important');
                n.justBefore(this);
                return;
                var p = this.getParent(function() {
                    return this !== document.documentElement
                            && this !== document.body
                            && getComputedStyle(this).overflow == 'auto'
                            || getComputedStyle(this).overflow == 'scroll';
                });
                p = p || this;
                var collRect = this.getClientRects();
                var rc = p.clientRect;
                if (rc) {
                    n.css({
                        // top: parseInt(p.offsetTop) + 'px !important',
                        // top:'0px',
    ...
    __prototype__.disable__ = function(on, progress) {
        ...
        if (on == true) {
            if (!n) {
                n = _$('div')
                        .css(
                                'background-repeat:no-repeat;background-position:50% 50%;background-color:rgba(255,255,255,0.5);position:absolute;transition: opacity 0.3s ease-in-out;');
                if (this === document.body)
                    this.add(n);
                else
                    this.offsetParent.add(n);
    ...
    __prototype__.disable_ = function(on, progress) {
        ...
        if (on == false) {
            if (n) {
                n
                        .css('transition: opacity 0.3s ease-in-out;transition: opacity 0.3s ease-in-out;opacity:0;');
                hto = setTimeout((function(n) {
                    n.remove();
                    this.___disabler = undefined;
                    delete this.___disabler;
        ...
        if (on == true) {
            if (!n) {
                n = _$('div')
                        .css(
                                'opacity:0;background-repeat:no-repeat;background-position:50% 50%;background-color: rgba(0,0,0,0);position:absolute;transition: opacity 0.3s ease-in-out;transition: opacity 0.3s ease-in-out;');
    ...
    __prototype__.canDrag = function(rem) {
        ...
        if (rem && !this.id)
            console
                    .error('Can not save this object state to cookies, id not specified');
        if (rem && this.id) {
        ...
        function mouseup(e) {
            ...
            b.style.cursor = "default";
            if (rem) {
                setCookie(d.id + "-drag-coords", d.style.left + ':'
                        + d.style.top);
            }
            e.preventDefault();
            e.stopPropagation();
    ...
    __prototype__.fillHeight = function() {
        ...
        while (p.offsetParent) {
            ...
            top += p.offsetTop;
            // console.info(h, top, window.innerHeight, p, p.offsetHeight,
            // p.style, getComputedStyle(p).top, p.offsetHeight, p.offsetTop);
            p = p.offsetParent;
        }
    ...
    /**
     * removes child elements and attributes raises cancellable clear event on
     * element
     * 
     * @function
     * @name HTMLElement#clear
     * @param {bool}
     *            destruct whether remove attributes and descendant nodes
     * 
     * @memberOf __prototype__
     */
     ...
     *            on
     * @param {Function}
     *            cb called on DOMElement instance with visibility parameter
     * @version 1.001
     * 
     * @memberOf __prototype__
     */
     ...
     * @param {Function}
     *            h called on DOMElement instance with visibility parameter
     * @memberOf __prototype__
     */
    __prototype__.show = function(h) {
        if (this.hasAttribute('data-control-event-display-params')) {
            ...
            // this.defaultStyle.display != "none" && this.defaultStyle.display
     ...
     * @param {bool}
     *            dispose either dispose attributes and object properties
     ...
     *            string
     * @param selector
     *            Function passes this as HTMLElement and arguments as arguments
     * @memberOf __prototype__
     */
     ...
     * @param {Function|bool}
     *            selector callback called with element context and arguments,
     *            if returns true elements filtered for selection else not
     *            selected if passed as true returns all tags with
    ...
    __prototype__.tags = function(tagName, selector) {
        ...
        for (var i = 0; i < col.length; i++) {
            var item = col[i];
            if (item instanceof HTMLElement
                    && item.tagName.toLowerCase() === tagName.toLowerCase()) {
                var select = true;
                if (selector instanceof Function)
                    select = selector.apply(item, args);
                if (select === true)
    ...
    __prototype__.setRectClick = function(ch, rc, t, e) {
        ...
        var x = [rc[0], rc[2]], y = [rc[1], rc[3]];
        if (isset(collRect) && collRect.length > 0) {
            rect = collRect[0];
            for (var i = 0; i < 2; i++) {
                x[i] = x[i] == null ? rect.right : (x[i] == 0
                        ? rect.left
                        : x[i] + (x[i] < 0 ? rect.right : rect.left));
                y[i] = y[i] == null ? rect.bottom : (y[i] == 0
                        ? rect.top
                        : y[i] + (y[i] < 0 ? rect.bottom : rect.top));
            }
            if (e.clientX >= x[0] && e.clientX < x[1] && e.clientY >= y[0]
                    && e.clientY < y[1]) {
    ...
    __prototype__.collectNodesWithDashedAttributes = function(name, collector) {
        ...
        var iterator = document.evaluate(xp, target, null,
                XPathResult.UNORDERED_NODE_ITERATOR_TYPE, null);
        while (attNode = iterator.iterateNext()) {
            var propName = attNode.name.split('-').last;
            value = collector instanceof Function
                    ? collector(attNode.ownerElement, attNode, propName)
                    : attNode.ownerElement.value;
            out[propName] = value;// attNode.nodeValue;
                    ...
                    try {
                        if (this.style.setProperty) {
                            ...
                            if (String.prototype.indexOf
                                    .call(val, '!important') != -1) {
                                val = String.prototype.split.call(val,
                                        '!important')[0];
                        ...
                        if (e.number == -2147024809) {
                            throw new Error("Invalid style attribute [" + item
                                    + ":" + style[item] + "]");
                        } else {
                            throw e;
                        }
     ...
     *            tg set tg as false to remove true to add class, string to
     *            replace first argument with this one
     * @param {bool}
     ...
     * @param {String}
     *            n name of the attribute. Can be colon semicolon name value
     *            pairs
     * @param {String}
    ...
    var __property__ = {
        ...
        clientSize : {
            get : function() {
                ...
                var h = this.clientHeight + parseInt(s.paddingTop)
                        + parseInt(s.paddingBottom);
                var w = this.clientWidth + parseInt(s.paddingLeft)
        ...
        placeHolder : {
            propertyOf : [HTMLTextAreaElement, HTMLInputElement,
                    HTMLSelectElement],
            get : function() {
        ...
        dirty : [
                 ...
                 */
                {
                    ...
                    get : function() {
                        ...
                        return this.getAttribute('defaultValue') != this
                                .getValue();
                    },
                    configurable : true
                },
                 ...
                 */
                {
                    ...
                    get : function() {
                        ...
                        // this.defaultValue != this.getValue(),
                        // 'initial=', this.defaultValue, 'value=', this
                        // .getValue(), this);
                        var option = this.options.item(0);
                        while (option) {
                            if (option.defaultSelected) {
                                // console.info('checkselect is dirty',
                 ...
                 */
                {
                    ...
                    get : function() {
                        if (this.type == "button")
                            return false;
                        // console.info('Input Dirty ?', this.defaultValue !=
                        // this.getValue(), this.defaultValue, this.getValue(),
                        // this.defaultChecked, this);
                        if (this.type == "checkbox" || this.type == "radio") {
                            return (this.getAttribute('defaultChecked') === "true"
                                    ? true
                                    : false) != this.checked;
                        }
                        return this.getAttribute('defaultValue') != this
                ...
                },
                {
                    ...
                    get : function() {
                        ...
                        if (this.initialElementsCount !== this.elementsCount)
                            return true;
                        var els = this.elements;
                        console.groupCollapsed('check Form dirty', this,
                                els.length, arguments);
                        for (var i = 0; i < els.length; i++) {
                            var item = els[i];
                            if (item.excluded)
                                continue;
        ...
        /**
         * returns element or parent that has data-control-draggable attribute
         * 
        ...
        /**
         * returns element or parent that has data-control-closure-scope
         * attribute
         * 
        ...
        /**
         * returns element or parent that has data-control-render-target
         * attribute
         * 
        ...
        computedBorder : {
            get : function() {
                var cs = getComputedStyle(this);
                var top = parseInt(cs.borderTop), bot = parseInt(cs.borderBottom), left = parseInt(cs.borderLeft), right = parseInt(cs.borderRight);
                top = isNaN(top) ? 0 : top;
                bot = isNaN(bot) ? 0 : bot;
        ...
        computedPadding : {
            get : function() {
                var cs = getComputedStyle(this);
                var top = parseInt(cs.paddingTop), bot = parseInt(cs.paddingBottom), left = parseInt(cs.paddingLeft), right = parseInt(cs.paddingRight);
                var v = top + bot;
                var h = left + right;
                return {
                    vertical : v,
        ...
        /**
         * returns element or parent that has data-event-target attribute
         * 
         * @name HTMLElement#eventTarget
         * @returns {HTMLElement}
         */
        ...
        /**
         * returns element have data-event-bridge attribute
         * 
        ...
        /**
         * returns element or parent that has data-event-bridge attribute
         * 
        ...
        /**
         * returns element or parent that has data-event-node attribute
         * 
        ...
        /**
         * returns value of element or parent that has data-event-target
         * attribute
         * 
         * @name HTMLElement#eventTarget
         * @returns {HTMLElement}
        ...
        /**
         * returns element or parent that has data-control-group attribute
         * 
        ...
        /**
         * returns element display state using data-control-event-display-params
         * attribute
         * 
        ...
        visible : {
            get : function() {
                if (this.hasAttribute('data-control-event-display-params')) {
                    var cls = this
                            .getAttribute('data-control-event-display-params');
                    cls = cls.object;
                    return this.className.split(' ').indexOf(cls[0]) !== -1;
                }
                var isv = this.style.display != 'none';
        ...
        offsetSize : {
            get : function() {
                var s = getComputedStyle(this);
                var h = this.offsetHeight + parseInt(s.marginTop)
                        + parseInt(s.marginBottom);
                var w = this.offsetWidth + parseInt(s.marginLeft)
    ...
    __prototype__.setTitle = {
        ...
        handle : function(v) {
            ...
            this.title = ts.join(' - ');
            /*
             * var t = this.title; var ts = t.split(' - '); ts.splice(0, 1);
             * ts.unshift(v); this.title = ts.join(' - ');
             */
        }
    };
    /**
    ...
    __prototype__.HTMLCollection = {
        ...
        handles : {
            /**
             * applies select array function on HTMLCollection
             * 
             * @function
            ...
            /**
             * converts HTMLCollection to array and returns it
             * 
             * @function
            ...
            if (item.hasOwnProperty('prototypeOf')) {
                ...
                for (var i = 0; i < ptos.length; i++) {
                    if (item.hasOwnProperty('handle')) {
                        ...
                        console.log('loadPrototype', key, utility.funcName
                                .call(ptos[i]), item.handle);
                    } else if (item.hasOwnProperty('handles')) {
                        for ( var hKey in item.handles) {
                            ptos[i].prototype[hKey] = item.handles[hKey];
                            console.log('loadPrototype', hKey, utility.funcName
                                    .call(ptos[i]), item.handles[hKey]);
                        }
                    } else
                        throw new Error(
                                'Prototype set configuration error for ' + key
                                        + '. Neighter handle nor handles set.');
                }
    ...
    domReady(function() {
        document.onkeydown = document.onkeypress = function(e) {
            if (e.keyCode == 8) {
                ...
                if (!((d.tagName.toUpperCase() == 'BODY') || (d.tagName
                        .toUpperCase() == 'HTML'))) {
                    doPrevent = (d.attributes['readonly']
                            || d.attributes['disabled'] || d.readonly || d.disabled);
                } else {
                    doPrevent = true;
                }
            } else {
...
function ___domReady(){
    ...
    /**
     * @id domReady
     *  fires when DOM Loaded or
     * document and libraries are ready
     */
...
function ___parser() {
    function rectH(rc, oh, t, e) {
        ...
        if (isset(collRect) && collRect.length > 0) {
            ...
            if (!src.hasAttribute('data-control-event-rect-disabled')
                    && e.clientX >= x[0] && e.clientX < x[1]
                    && e.clientY >= y[0] && e.clientY < y[1]) {
                ...
                if (e.type == 'click') {
                    ...
                    if (src['data-control-event-rect-handle']
                            || src
                                    .hasAttribute('data-control-event-rect-handle')) {
                        var eh = src['data-control-event-rect-handle']
                                || src
                                        .getAttribute('data-control-event-rect-handle');
                ...
                if (src.___title) {
                    ...
                    src.title = '';
                if (e.type == 'click' && oh) {
                    var clh = oh instanceof Function
                            ? oh
                            : (typeof oh == 'string' ? oh.object : oh);
                    clh.call(src, e);
                }
            }
        }
    ...
    var dataAttributeParser = {
        ...
        event : {
            ...
            hash : {
                 ...
                 */
                listener : function(v) {
                    // v = addSourceUrl.call(this, v, 'listener');
        ...
        control : {
            event : {
                ...
                listen : function(v, n, o, a, ns) {
                    ...
                    ca.event.listen(o.name, o.handle.bind(this), o.target
                            || this, false);
                    this.removeAttribute(n);
                }
            },
            format : {
                checkbox : function(v, n, oh) {
                    ...
                    var data = typeof o === 'function'
                            ? o.call(this)
                            : (typeof o === 'object' ? o : v.split(','));
                    if (data.length === 1)
                        data.push('');
                    ...
                    ca.event.listen('click', function(e) {
                        ...
                        this.firstElementChild.value = this.checked
                                ? data[0]
                                : data[1];
                        ca.event.raise('change', {}, this.firstElementChild,
                                true);
                    }.bind(this), this, false);
                ...
                eof : function(v, n, oh) {
                    if (this instanceof HTMLTableElement === false)
                        return;
                    var text = (typeof v === "string" && v.length)
                            ? v
                            : 'Kayıt mevcut değil';
                    ...
                    set : function(v, n, oh) {
                        ...
                        if (data && data.trim().length > 0) {
                            ...
                            for (var i = 0; i < tempStyle.sheet.cssRules.length; i++) {
                                ...
                                window.__parsedElementsCss.sheet.insertRule(
                                        tr.cssText, 0);
            ...
            parser : {
                constructor : function(v, n) {
                    var att = this
                            .getAttribute('data-control-parser-arguments');
                    if (!att) {
            ...
            constructor : function(v, n) {
                var att = this
                        .getAttribute('data-control-constructor-arguments');
                if (!att) {
                    console.error('arguments for ' + n + ' not set');
                    return;
                }
            ...
            tooltip : function(v) {
                ...
                if (opts instanceof Object) {
                    ...
                    var isFocus = this.tagName === 'INPUT'
                            || this.tagName === 'TEXTAREA'
                            || this.tagName === 'SELECT';
                    pe = this.hasAttribute('data-control-tooltip-event')
                            ? this.getAttribute('data-control-tooltip-event')
                            : (isFocus ? 'focus' : 'mouseover');
                    var he = this
                            .hasAttribute('data-control-tooltip-hide-event')
                            ? this
                                    .getAttribute('data-control-tooltip-hide-event')
                            : isFocus ? 'blur' : 'mouseout';
                    sh = (function(v, he) {
                        this.setToolTip({
                            message : v,
                            hideEvent : he
            ...
            /**
             * use compare for eauality and unequal for unequality comparison
             * //TODO:use events in validation
             */
            validators : function(v) {
                var ar = v instanceof Array ? v : v.object;
                this.validate = function(ar) {
                    console.info('validating with inline validators', this,
                            arguments);
                    this.isValid = true;
                                    ...
                                    function(o, i, el) {
                                        ...
                                        if (o.compare || o.unequal) {
                                            ...
                                            if (o.pattern instanceof Function)
                                                ...
                                                var r = o.pattern instanceof RegExp
                                                        ? o.pattern
                                        ...
                                        if (test instanceof Function) {
                                            if (!test(el.getValue())) {
                                                ...
                                                var availHeight = getClientHeight(), rect = target.clientRect;
                                                if (isset(rect)
                                                        && (rect.top > availHeight || rect.top < 0)) {
                                                    ...
                                                    window
                                                            .scrollTo(
                                                                    ...
                                                                    bTop
                                                                            + parseInt(rect.top
                                                                                    - parseInt(availHeight / 2)));
                                                }
                                                ca.event.raise('error-set', o,
                                                        target, true);
                                                target.focus();
                                                target.setError({
                                                    message : o.message,
            ...
            on : {
                ready : function(v, n, o, a, ns) {
                    // v = addSourceUrl.call(this, v, 'ready');
                    var fn = v.toObject(this, false);
                    fn.call(this,
                            this.closure ? this.closure.scope : undefined, ns,
                            a);
            ...
            url : function(v, n, oh) {
                ...
                var target = this.hasAttribute(targetAttName) ? this
                        .getAttribute(targetAttName) : false;
                this.removeAttribute(n);
                ...
                ca.event.listen('click',
                        function(e) {
                            ...
                            if (pass) {
                                ...
                                e.stopPropagation();
                                e.stopImmediatePropagation();
                                ca.event.raise('region-click', data, e.target,
                                        true);
                            }
                        }, this, false);
            },
    ...
    use(parserNs);
    function parseDataAttribute(attName, h) {// var xp = "count(@" + attName
        // + ")";
        ...
        // var fn = attName.replace(/((\-)?([^\-]+))/g, function(match, p1, p2,
        // p3)
        // {
    ...
    function addSourceUrl(v, s) {
        if (!DEVELOPMENT_MODE)
            return v;
        s = s ? s + '-' : '';
        var uniq = this.id && this.id.length > 0 ? this.id : (this.name
                && this.name.length > 0 ? this.name : uniqid(false));
        var re = /\/\/#\s*sourceURL\s*=\s*.+/i;
        if (!re.test(v)) {
            re = /(function\s*\(\w*\)\s*\{)(.*)/;
            v = v.replace(re, '$1/\/# sourceURL=ca-' + s
                    + this.tagName.toLowerCase() + '-' + uniq + '.js\r$2')
                    + '\n';
        } else {
    ...
    stubTraverser.prototype.setProperty = function(name, v) {
        if (/\w+\-\w+/.test(name)) {
            ...
            if (typeof v === "string"
                    && (v.search(/[\(\)\{\}]/i) !== -1 || (v
                            .search(/(\w+\.)+\w/i) !== -1 && v.search(/[\s]/i) === -1))) {
                try {
                    obj = v.toString().toObject(control, false);
                } catch (e) {
                    var err = e.constructor('Error in script for ' + this.stub
                            + '.' + name + ' :\n' + e.message);
                    throw err;
                }
            } else
    ...
    var dataAttributeStubHandlers = {
        ...
        'data-control-function' : {
            setProperty : function(name, code, a, ns) {
                ...
                // code = addSourceUrl.call(this, code, fn);
                this[name] = obj;
            }
        }
    };
    ...
    function parseStubAttributes(a, ns) {
        ...
        for ( var key in dataAttributeStubHandlers) {
            ...
            if (attNode) {
                ...
                instance = stubObj instanceof Function ? new stubObj(this, key,
                        a, ns) : {
                    setProperty : stubObj.setProperty ? stubObj.setProperty
                            .bind(this) : undefined,
                };
            }
            while (attNode) {
                ...
                var code = attNode.value;
                var handler = instance.setProperty, arg = code;
            ...
            /*
             * try { } catch (er) { console.error(er); if ( er instanceof
             * evalError) throw er; }
             */
        }
    }
    ...
    function parseElement(a, ns) {
        ...
        if (this.validate instanceof Function) {
            if (this.addEventListener) // Opera - Firefox - Google Chrome
            {
                this
        ...
        if (this instanceof HTMLInputElement) {
            ...
            if ((this.type == 'checkbox' || this.type == 'radio')) {
                ...
                if (!this.hasAttribute('defaultChecked'))
                    this.setAttribute('defaultChecked', this
        ...
        if (this instanceof HTMLElement) {
            ...
            if (this.hasAttribute('data-control-verb-transform')) {
                var d = getObjectFromAttribute(this,
                        'data-control-verb-transform', true);
                ca.event.listen('personalize', function(d, e) {
                    function preg(dc) {
                        var tg = 'innerHTML';
                        for (var i = 0, n = dc.params.length; i < n; ++i) {
                            this[tg] = this[tg].replace(dc.params[i][0],
                                    dc.params[i][1]);
                        }
                    }
            ...
            if (this.hasAttribute('data-control-event-rect')) {
                ...
                var t = getStringFromAttribute(this,
                        'data-control-event-rect-title', true);
                var rc = getObjectFromAttribute(this,
                        'data-control-event-rect', true);
                this.addEventListener('mousemove', rectH.bind(this, rc,
                        undefined, t), true);
                if (oClk)
                    this.addEventListener('click', rectH.bind(this, rc, oClk,
                            undefined), true);
            }
        }
...
function ___event()
{
    ...
    var event = {
         ...
         * @param {eventCallBack} callBack listener handle
         * @param {Object?} [target=window] event target
         * @param {bool?} [useCapture=false] useCapture
         */
         ...
         * @memberOf ca.event
         * @param {Object} name event name
         * @param {Object} arguments passed as event.detail
         * @param {Object?}   [target=window] event target
         * @param {bool?}  [bubbles=false] whether event bubbles
         * @param {bool?} [cancellable=true]  whether event cancellable
         */
        raise : function(n, a, t, b, c,doTrace)
        {
            ...
            if (window.CustomEvent) {
                ...
                t = t || window;
if(doTrace===true)      console.trace('%c' + n , 'font-weight:bold;color:red;font-size:larger;');
                var cancelled = !t.dispatchEvent(event);
                return cancelled;
                //if(cancelled) event.target.removeEventListener(event.type, arguments.callee);
            }
        }
    };
use('ca');
